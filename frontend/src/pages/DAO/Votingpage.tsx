...
const VotingPage: React.FC = () => {
  ...
  const [usersVotingDetails, setUsersVotingDetails] = useState<UserVotingDetails[]>([]);
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-gray-600">Please provide a DAO UUID in the URL path</p>
            <p className="text-sm text-gray-500 mb-4">Example: /voting/your-dao-uuid</p>
            <Button 
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">
                {usersVotingDetails.filter(u => u.hasAttestation && u.attestationResult).length}
              </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full ${currentAccount ? 'bg-green-500' : 'bg-red-500'}`}></div>
                <span className="text-sm">
                  {currentAccount ? `Connected: ${currentAccount.slice(0, 6)}...${currentAccount.slice(-4)}` : 'Wallet not connected'}
                </span>
              </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-semibold">Users to Vote On ({usersVotingDetails.length})</h2>
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 mt-2">Users need to be added to the DAO first</p>
            </CardContent>
          </Card>
        ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <CardTitle className="text-lg">
                        {userDetails.userAddress.slice(0, 6)}...{userDetails.userAddress.slice(-4)}
                      </CardTitle>
                      <CardDescription>
                        Votes: {userDetails.totalVotes} | Yes: {userDetails.yesVotes} | No: {userDetails.noVotes}
                      </CardDescription>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {userDetails.hasAttestation && (
                        <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                          ...
                          {userDetails.attestationResult ? '✓ APPROVED' : '✗ REJECTED'}
                        </div>
                      )}
                ...
                <CardContent>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="text-center p-3 bg-green-50 rounded-lg">
                      <div className="text-xl font-bold text-green-600">{userDetails.yesVotes}</div>
                      <div className="text-sm text-green-700">Yes Votes</div>
                    </div>
                    <div className="text-center p-3 bg-red-50 rounded-lg">
                      <div className="text-xl font-bold text-red-600">{userDetails.noVotes}</div>
                      <div className="text-sm text-red-700">No Votes</div>
                    </div>
                    <div className="text-center p-3 bg-blue-50 rounded-lg">
                      <div className="text-xl font-bold text-blue-600">{userDetails.totalVotes}</div>
                  ...
                  {userDetails.hasAttestation && (
                    <div className="bg-gray-50 p-4 rounded-lg mb-4">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div><strong>Result:</strong> {userDetails.attestationResult ? 'Approved' : 'Rejected'}</div>
                        <div><strong>Created:</strong> {formatTimestamp(userDetails.createdAt)}</div>
                        {userDetails.attestedAt > 0 && (
                          <div><strong>Attested:</strong> {formatTimestamp(userDetails.attestedAt)}</div>
                        )}
                  ...
                  {isVerifier && !userDetails.hasAttestation && (
                    <div className="border-t pt-4">
                      ...
                      <div className="space-y-3">
                        <textarea
                          value={selectedUser === userDetails.userAddress ? voteReason : ''}
                          onChange={(e) => {
                        ...
                        <div className="flex gap-3">
                          <Button
                            onClick={() => handleCastVote(userDetails.userAddress, VoteOption.YES)}
                            disabled={loading}
                          ...
                          <Button
                            onClick={() => handleCastVote(userDetails.userAddress, VoteOption.NO)}
                            disabled={loading}
                            variant="destructive"
                            className="flex-1"
                          >
                  ...
                  {!isVerifier && currentAccount && (
                    <div className="border-t pt-4">
                      <div className="bg-yellow-50 text-yellow-800 p-3 rounded-lg text-sm">
                        <strong>Note:</strong> You are not a verifier for this DAO and cannot vote.
                      </div>
                    </div>
                  )}
                  ...
                  {!currentAccount && (
                    <div className="border-t pt-4">
                      <div className="bg-red-50 text-red-800 p-3 rounded-lg text-sm">
                        <strong>Connect your wallet</strong> to participate in voting.
                      </div>
                    </div>
                  )}
                  ...
                  <div className="border-t pt-4 mt-4">
                    <Button
                      onClick={() => setExpandedUser(isExpanded ? '' : userDetails.userAddress)}
                      variant="ghost"
                      size="sm"
                      className="mb-3"
                    >
                      {isExpanded ? '🔼 Hide' : '🔽 Show'} Individual Votes ({userDetails.votes.length})
                    </Button>
                    
                    {isExpanded && (
                      <div className="space-y-2 max-h-60 overflow-y-auto">
                        {userDetails.votes.length === 0 ? (
                          <p className="text-gray-500 text-sm italic">No votes cast yet</p>
                        ) : (
                            ...
                            <div key={index} className="bg-gray-50 p-3 rounded border-l-4 border-l-blue-500">
                              <div className="flex justify-between items-start mb-1">
                                <span className="text-sm font-medium">
                                  {vote.verifier.slice(0, 6)}...{vote.verifier.slice(-4)}
                                </span>
                                <div className="flex items-center gap-2">
                              ...
                              {vote.reason && (
                                <p className="text-sm text-gray-600 mt-1 italic">"{vote.reason}"</p>
                              )}
                            </div>
                          ))
                        )}
