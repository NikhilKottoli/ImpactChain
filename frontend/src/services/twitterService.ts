...
class TwitterService {
  ...
  private generateOAuthSignature(
    ...
    const oauthParams = {
      ...
      oauth_version: '1.0',
      ...(this.credentials.accessToken && { oauth_token: this.credentials.accessToken }),
    };
  ...
  private async getBearerToken(): Promise<string> {
    try {
      const credentials = btoa(`${this.credentials.apiKey}:${this.credentials.apiSecret}`);
  ...
  async createPost(postData: TwitterPostData): Promise<TwitterPostResponse> {
    try {
      ...
      const twitterUrl = `${TWITTER_CONFIG.endpoints.intent}?${params.toString()}`;
  ...
  async shareImpactChain(additionalText?: string, imageFile?: File): Promise<TwitterPostResponse> {
  ...
  async shareSocialPost(postTitle: string, postDescription: string, imageFile?: File): Promise<TwitterPostResponse> {
  ...
  // Share a campaign
  async shareCampaign(campaignTitle: string, campaignDescription: string, imageFile?: File): Promise<TwitterPostResponse> {
